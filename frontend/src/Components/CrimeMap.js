...
const CrimeMap = function () {
  ...
  const [mapCenter] = useState([6.9061, 79.9696]); // Static default location
  ...
  return (
    <div className="w-[90vw] mx-auto p-2 rounded-1xl shadow-lg bg-gray-600 m-2 relative z-10">
      ...
      <div className="relative flex justify-end p-5 z-0">
        <Link to="/locationListUser">
          <button className="bg-amber-500 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded mr-5">
            Location List
          </button>
        </Link>
        <button
