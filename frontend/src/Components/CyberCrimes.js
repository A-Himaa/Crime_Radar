    ...
    const fetchArticles = async () => {
      try {
        ...
        const filtered = res.data.filter(
          (article) =>
            article.title?.toLowerCase() === "cyber crimes" ||
            article.category?.toLowerCase() === "cyber crimes"
        );
      ...
      <div className="w-full max-w-5xl w-[80vw] mt-[15vh]">
        ...
        <div className="flex justify-center mb-10 border-b-4 border-amber-600 pb-4">
          <h1 className="text-4xl font-extrabold text-center">
            <span className="text-amber-600">C</span>yber{" "}
        ...
        {articles.length > 0 ? (
          ...
          <p className="text-center text-red-600 text-lg mt-20">
            No Cyber Crimes articles found.
          </p>
        )}
      </div>
    </div>
