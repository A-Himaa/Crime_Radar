...
const LocationList = () => {
  ...
  useEffect(() => {
    function getLocation() {
      axios
        .get("http://localhost:8070/locations/locationList")
        .then((res) => {
  ...
  const onDeleteClick = async (locationId) => {
    ...
    if (confirmed) {
      await axios.delete(
        `http://localhost:8070/locations/delete/${locationId}`
      );
    ...
    .filter(
      (location) =>
        ...
        (location.coordinates &&
          location.coordinates[0]?.toString().includes(searchTerm)) || // Latitude
        (location.coordinates &&
          location.coordinates[1]?.toString().includes(searchTerm)) // Longitude
    )
      ...
      <div className="flex flex-row gap-6">
        ...
        <div className="w-1/2 overflow-x-auto">
          ...
          <div className="flex justify-end mt-4">
            <button
              onClick={() => downloadLocationPDF(filteredLocation)}
