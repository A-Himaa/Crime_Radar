...
const ViolentCrimes = () => {
    ...
    const fetchArticles = async () => {
      try {
        ...
        const filtered = res.data.filter(
          (article) =>
            article.title?.toLowerCase() === "violent crimes" ||
            article.category?.toLowerCase() === "violent crimes"
        );
  ...
  const downloadPDF = (article) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text("Violent Crime Awareness Report", pageWidth / 2, currentY, {
      align: "center",
    });
    ...
    // Save PDF
    doc.save(`Violent_Crime_Article_${article.article_id || "Unknown"}.pdf`);
  };
      ...
      <div className="w-full max-w-5xl w-[80vw] mt-[15vh]">
        ...
        <div className="flex justify-center mb-10 border-b-4 border-amber-600 pb-4">
          <h1 className="text-4xl font-extrabold text-center">
            <span className="text-amber-600">V</span>iolent{" "}
        ...
        {articles.length > 0 ? (
          ...
          <p className="text-center text-red-600 text-lg mt-20">
            No Violent Crimes articles found.
          </p>
        )}
      </div>
    </div>
